...
class TestOnIncidentCreatedDatadog:
    ...
    @patch("firetower.incidents.hooks.logger")
    @patch("firetower.incidents.hooks._create_status_channel")
    @patch("firetower.incidents.hooks._invite_oncall_users")
    @patch("firetower.incidents.hooks._page_if_needed")
    @patch("firetower.incidents.hooks.DatadogService")
    @patch("firetower.incidents.hooks._slack_service")
    def test_logs_warning_when_no_channel_for_bookmark(
        self,
        mock_slack,
        mock_datadog_cls,
        mock_page,
        ...
        # Slack channel creation fails -> channel_id is None, but notebook
        # creation should still proceed and warn that there is no place to
        # bookmark it.
        mock_slack.create_channel.return_value = None
        ...
        assert any(
            "https://app.datadoghq.com/notebook/abc123" in msg
            and "no Slack channel" in msg
    ...
    @patch("firetower.incidents.hooks.DatadogService")
    @patch("firetower.incidents.hooks._slack_service")
    def test_private_incident_with_no_channel_does_not_crash(
        self,
        mock_slack,
        mock_datadog_cls,
        mock_page,
