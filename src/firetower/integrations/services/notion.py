...
class NotionService:
    ...
    def add_timeline_to_page(self, page_id: str, timeline_markdown: str) -> None:
        now = datetime.now(tz=UTC)
        date_time_str = now.strftime("%Y-%m-%d %H:%M:%S UTC")
        toggle: dict[str, Any] = {
            "type": "toggle",
            "toggle": {
                "rich_text": [
                    {
                        "type": "text",
                        "text": {
                            "content": f"AI-Generated Timeline (Draft) as of {date_time_str}"
                        },
                    }
                ],
                "children": [
                            ...
                            "rich_text": [
                                {
                                    ...
                                    "text": {
                                        "content": (
                                            "This timeline was automatically generated by AI "
                                            "from Slack messages. Please review and edit for accuracy."
                                        )
                                    },
                                }
                            ],
        ...
        if not response or not response.get("results"):
            raise RuntimeError(
                f"Failed to append AI timeline toggle to Notion page {page_id}"
            )
        toggle_id = response["results"][0]["id"]
    ...
    for line in markdown_text.split("\n"):
        ...
        if line.startswith("## "):
            blocks.append(
                {
                    "type": "heading_2",
                    "heading_2": {
                        "rich_text": _parse_timestamps_to_rich_text(line[3:].strip())
                    },
                }
            )
            ...
            blocks.append(
                {
                    "type": "bulleted_list_item",
                    "bulleted_list_item": {
                        "rich_text": _parse_timestamps_to_rich_text(content)
                    },
                }
            )
        else:
            blocks.append(
                {
                    "type": "paragraph",
                    "paragraph": {"rich_text": _parse_timestamps_to_rich_text(line)},
                }
            )
    return blocks
